  ...
  <div class='login'>
    <el-card class='login-card'>
      <el-header class="header-box">
        <h2 class="login-title"><i class="logo"></i></h2>
      </el-header>
      <!-- 登录表单 -->
      <el-form ref='loginForm' :model="loginForm" :rules="loginRules" style='margin-top:20px' label-width="auto">
    ...
    methods: {
      ...
      async axiosLogin(params) {
        ...
        if (200 == res.code) {
          ...
          let response = getInfo(res.data.token);
          console.log(JSON.stringify(response))
          if(200==response.code){
            window.localStorage.setItem('user-info',response.data);
          }
